      ...
      <CardContent className="space-y-3 sm:space-y-4 p-4 sm:p-6 pt-0">
        <div className="space-y-2">
          <div className="flex justify-between items-center mb-1">
            <span className="text-xs sm:text-sm text-muted-foreground">{t("monthlyBudget")}</span>
            <span className="text-sm sm:text-base font-semibold">{monthlyIncome.toFixed(3)} {t("kwd")}</span>
          </div>
          <Progress
          ...
          <div className="flex justify-between items-center">
            <span className="text-xs sm:text-sm text-muted-foreground">{t("spentThisCycle")}</span>
            <span
              className={`text-base sm:text-lg font-bold ${
                isOverBudget ? "text-destructive" : "text-foreground"
              }`}
            >
              {cycleExpenses.toFixed(3)} {t("kwd")}
            </span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 p-2.5 sm:p-3 rounded-lg bg-muted/50 border border-border">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-[10px] sm:text-xs text-muted-foreground">{t("currentCycle")}</p>
            <p className="text-xs sm:text-sm font-medium truncate">
              {format(startDate, "MMM d")} - {format(endDate, "MMM d, yyyy")}
            </p>
          </div>
          <div className="text-right flex-shrink-0">
            <p className="text-[10px] sm:text-xs text-muted-foreground">{t("daysLeft")}</p>
          ...
          <div className="flex items-start gap-2 p-2.5 sm:p-3 rounded-lg bg-destructive/10 border border-destructive/20">
            ...
            <div>
              <p className="font-semibold text-destructive text-xs sm:text-sm">
                {t("overBudgetThisCycle")}
              </p>
              <p className="text-[10px] sm:text-xs text-destructive/80">
                {t("exceededBudgetBy")}{" "}
                {Math.abs(remaining).toFixed(3)} {t("kwd")}
              </p>
            </div>
          </div>
        ) : (
          <div className="p-3 sm:p-4 rounded-lg bg-success/10 border border-success/20">
            <p className="text-xs sm:text-sm text-muted-foreground mb-1">{t("remainingThisCycle")}</p>
            <p className="text-xl sm:text-2xl font-bold text-success">
              {remaining.toFixed(3)} {t("kwd")}
            </p>
          </div>
        )}
        ...
        <div className="pt-2.5 sm:pt-3 border-t border-border space-y-1.5 sm:space-y-2">
          <div className="flex justify-between items-center">
            <span className="text-xs sm:text-sm text-muted-foreground">
              {t("recommendedDailySpend")}
            </span>
            <span className="text-sm sm:text-base font-semibold">
              {dailyBudget.toFixed(3)} {t("kwd")}/{t("day")}
            </span>
          </div>
          <div className="flex justify-between items-center">
            <span className="text-xs sm:text-sm text-muted-foreground">
